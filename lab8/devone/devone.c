...
#endif
{
	struct devone_data *devone = (struct devone_data *)dev;
	unsigned long flags;

	spin_lock_irqsave(&devone->lock, flags);
	printk("%s called\n", __func__);
	devone->timeout_done = 1;
	wake_up(&devone->wait);
	spin_unlock_irqrestore(&devone->lock, flags);
}

#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0)
static void devone_irq(unsigned long arg)
#else
static void devone_irq(struct timer_list *arg)
#endif
{
	struct devone_data *devone1 = (struct devone_data *)dev;
	unsigned long flags;

	spin_lock_irqsave(&devone1->lock, flags);
	printk("%s called\n", __func__);
	devone1->dma_done = 1;
	wake_up(&devone1->wait);
	spin_unlock_irqrestore(&devone1->lock, flags);
}
